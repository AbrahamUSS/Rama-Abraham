...
CREATE TABLE IF NOT EXISTS `ASIGNACION_ROL` (
	`id_asignRol` INT NOT NULL AUTO_INCREMENT,
	`id_administrativo` INT NOT NULL,
	`id_rol` INT NOT NULL,
	`fecha_inicio` DATETIME NOT NULL,
	`fecha_fin` DATETIME,
	PRIMARY KEY (`id_asignRol`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
ALTER TABLE `ASIGNACION_ROL`
	ADD CONSTRAINT `fk_asignrol_administrativo` FOREIGN KEY (`id_administrativo`) REFERENCES `ADMINISTRATIVO`(`id_administrativo`)
	ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE `ASIGNACION_ROL`
	ADD CONSTRAINT `fk_asignrol_rol` FOREIGN KEY (`id_rol`) REFERENCES `ROL`(`id_rol`)
	ON UPDATE CASCADE ON DELETE RESTRICT;
